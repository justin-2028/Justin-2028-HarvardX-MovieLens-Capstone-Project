...
### IV. Movie + User Effects Regularization Model
...
In order to construct a movie recommendation algorithm with a sufficiently low RMSE, I employed an iterative applied machine learning approach. Beginning with naive approaches, I developed models that branched out to other features such as movie and user effects, and applied the regularization technique to further cut down the RMSE value.
...
We could also see drastic RMSE reduction with the use of a larger dataset. The MovieLens 10M dataset could be swapped for the 25M dataset offered on the 


 dataset size and the computing power of the machine are the two most difficult obstacles we can run across while putting those extra models into use and continuing with the overall improvements.
...
We could also build and train other models such as KNN, Matrix Factorization, and PCA. Some of these algorithms could improve the RMSEs, others could improve the Accuracy.
...
My MovieLens Project encounter several limitations that I hope to address in the future, perhaps through the assistance of my peers. 

The training and modeling are perform on a laptop that has limited memory and processing powers. I have attempted to evaluate k-NN models, but all of them failed due to hardware limitation. 

According to MovieLens website, the are fews modeling that manage to achieve RMSE less than 0.7. Most of these modeling are performed on specialize program.
