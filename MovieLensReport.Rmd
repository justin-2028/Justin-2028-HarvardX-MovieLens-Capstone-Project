...
The Movielens dataset will be split into 2 subsets: a training subset (edx) and a testing subset (Validation).
...
ages_effects <- edx %>% 
  ...
  left_join(user_effects, by='userId') %>%
  group_by(ages) %>%
  summarize(b_a = mean(rating - mu - b_m - b_u))
...
predicted_ratings_m_u_a <- validation %>% 
  ...
  mutate(prediction = mu + b_m + b_u + b_a) %>%
  pull(prediction)
